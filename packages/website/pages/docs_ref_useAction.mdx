...
The `useAction` hook allows you to fetch data from an Action function written alongside a component. Action functions have access to server-only resources such as databases that you provide and their code is never exposed to the browser.
...
Any additional arguments from the action execution function are passed as additional arguments to your action function.
