...
Behind the scenes, Firebolt takes care of bundling, server rendering, parallel streaming, routing, data access and so much more.
...
They work seamlessly with SSR and on the client, and are fully compatible with Suspense and Error Boundaries, just how React intended.

Forget about building API's and infrastructure and just focus on your product. Of course if you still need to provide a public API for your users to consume, we have [API Routes](/docs/ref/api-js).
...
You no longer have to wrangle with hacky, flaky workarounds to get good runtime CSS-in-JS that works with SSR and streaming flawlessly.
...
Forget about framework specific `<Head>` components, thanks to React 19 you can use `<title>` and `<meta>` tags anywhere in your app and they'll automatically be hoisted to the `<head>` element.
...
We baked cookie management into the core of Firebolt allowing you interact with them easily on both the server and client.
...
In this example, updating the `country` cookie will also update any other components observing its value.

You can even update cookies on the server inside Loaders & Actions and they will instantly update on the client.

### Markdown & MDX

Firebolt supports MDX for writing pages with markdown by simply adding a `.mdx` page.

```jsx title='pages/docs_intro.mdx' lineNumbers
# Introduction
...
It's easy to create shared layouts by simply nesting routes together. As an example, we can nest our MDX page from above into a shared navigation component like this:
...
To get started, check out our [documentation](/docs) and [github repo](https://github.com/firebolt-dev) and follow us on X to stay up to date.

If you are as excited about the future of web development as we are, be sure to share your thoughts on social media and share this post!
