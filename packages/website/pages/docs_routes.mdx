...
The `routes` directory is a unique way of organising all of your static and dynamic content that needs to be available via URL.

## Static Files

At it's most basic, the `routes` directory can serve static assets such a fonts, robots files and images by simply placing them here.
...
You can [configure](/docs/ref/config) MDX with plugins to provide syntax highlighting or support github flavored markdown, whatever you need.

You can also customize the components rendered by MDX with an [MDXProvider](/docs/ref/mdx-provider)

## Summary

The `routes` folder is a simple yet powerful concept that allows you to better organise your app and keep things as clean as possible.
