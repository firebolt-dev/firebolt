...
The `routes` directory is an intuitive yet unique way of organizing all of your static and dynamic content that needs to be available at a URL.

## Static Files

At it's most basic, the `routes` directory can serve static assets such as fonts, robots files and images by simply placing them there.
...
Yup, we also support MDX pages out of the box, allowing you to write your content with super powered markdown. Simply use the `.mdx` extension.
